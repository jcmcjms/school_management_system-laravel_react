...
import { AlertTriangle, Check, ChefHat, Clock, DollarSign, Package, QrCode, Utensils } from 'lucide-react';
...
const formatTimeFull = (d: string) =>
    new Date(d).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true });
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Dashboard', href: '/staff/dashboard' }];

export default function StaffDashboard() {
    const { pendingOrders, preparingOrders, readyOrders, servedToday, lowStockInventory, statusCounts, todayRevenue } =
    ...
    useEffect(() => {
        const id = setInterval(() => {
            router.reload({
                only: ['pendingOrders', 'preparingOrders', 'readyOrders', 'servedToday', 'statusCounts', 'lowStockInventory', 'todayRevenue'],
            });
        }, 10000);
    ...
    const handleRedeemQR = (e: React.FormEvent) => {
        ...
        router.post(
            ...
            { qr_code: qrCode },
            {
                ...
                onSuccess: () => {
                    setQrCode('');
                    setQrSuccess('Reservation redeemed successfully!');
    ...
    const handleConfirmPayment = (e: React.FormEvent) => {
        ...
        router.post(
            `/staff/orders/${showPayment.id}/confirm-payment`,
            {
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'queue' ? 'bg-primary text-primary-foreground' : 'bg-card hover:bg-accent border'}`}
                        >
                            <ChefHat className="mr-2 inline h-4 w-4" /> Order Queue
                        </button>
                        <button
                            onClick={() => setActiveTab('history')}
                            className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'history' ? 'bg-primary text-primary-foreground' : 'bg-card hover:bg-accent border'}`}
                        >
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-5">
                    <div className="rounded-lg border border-yellow-500 bg-yellow-50 p-4 shadow-sm dark:bg-yellow-950">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-medium text-yellow-700 dark:text-yellow-300">New Orders</p>
                    ...
                    <div className="rounded-lg border border-blue-500 bg-blue-50 p-4 shadow-sm dark:bg-blue-950">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-medium text-blue-700 dark:text-blue-300">Preparing</p>
                                <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">{statusCounts.preparing}</p>
                            </div>
                    ...
                    <div className="rounded-lg border border-green-500 bg-green-50 p-4 shadow-sm dark:bg-green-950">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-medium text-green-700 dark:text-green-300">Ready</p>
                                <p className="text-2xl font-bold text-green-700 dark:text-green-300">{statusCounts.ready}</p>
                            </div>
                    ...
                    <div className="bg-card rounded-lg border p-4 shadow-sm">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-muted-foreground text-xs font-medium">Served Today</p>
                    ...
                    <div className="bg-card rounded-lg border p-4 shadow-sm">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-muted-foreground text-xs font-medium">Today Revenue</p>
                ...
                {activeTab === 'queue' ? (
                    <>
                        ...
                        <div className="bg-card rounded-lg border p-6 shadow-sm">
                            <h2 className="mb-4 flex items-center gap-2 text-xl font-semibold">
                                <QrCode className="h-5 w-5" /> Scan Reservation QR
                            </h2>
                            ...
                            <div className="bg-card rounded-lg border shadow-sm">
                                ...
                                <div className="max-h-[500px] overflow-y-auto p-3">
                                    {pendingOrders.length === 0 ? (
                                        <p className="text-muted-foreground py-4 text-center text-sm">No new orders</p>
                                    ) : (
                                                    ...
                                                    <div className="mb-2 flex items-center justify-between">
                                                        <div>
                                                            ...
                                                            <p className="text-muted-foreground text-xs">
                                                                {formatTime(order.created_at)} — {order.user?.name}
                                                            </p>
                                                        </div>
                                                        <div className="text-right">
                                                            <p className="font-medium">₱{formatPrice(order.total)}</p>
                                                            <span
                                                                className={`inline-block rounded-full px-1.5 py-0.5 text-[10px] font-medium ${paymentBadge(order)}`}
                                                            >
                                                                {paymentLabel(order)}
                                                            </span>
                                                        </div>
                            ...
                            <div className="bg-card rounded-lg border shadow-sm">
                                ...
                                <div className="max-h-[500px] overflow-y-auto p-3">
                                    {preparingOrders.length === 0 ? (
                                        <p className="text-muted-foreground py-4 text-center text-sm">No orders preparing</p>
                                    ) : (
                                        <div className="space-y-3">
                                            {preparingOrders.map((order) => (
                                                <div
                                                    key={order.id}
                                                    className="rounded-md border border-blue-300 bg-blue-50/50 p-3 dark:border-blue-800 dark:bg-blue-950/50"
                                                >
                                                    <div className="mb-2 flex items-center justify-between">
                                                        <div>
                                                            <p className="font-semibold">{order.order_number}</p>
                            ...
                            <div className="bg-card rounded-lg border shadow-sm">
                                ...
                                <div className="max-h-[500px] overflow-y-auto p-3">
                                    {readyOrders.length === 0 ? (
                                        <p className="text-muted-foreground py-4 text-center text-sm">No orders ready</p>
                                    ) : (
                                        <div className="space-y-3">
                                            {readyOrders.map((order) => (
                                                <div
                                                    key={order.id}
                                                    className="rounded-md border border-green-300 bg-green-50/50 p-3 dark:border-green-800 dark:bg-green-950/50"
                                                >
                                                    <div className="mb-2 flex items-center justify-between">
                                                        <div>
                                                            <p className="font-semibold">{order.order_number}</p>
                            ...
                            <div className="rounded-lg border border-red-500 bg-red-50 p-6 shadow-sm dark:bg-red-950">
                                ...
                                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                                    {lowStockInventory.map((item) => (
                                        <div
                                            key={item.id}
                                            className="flex items-center justify-between rounded-md border border-red-300 bg-white p-2 dark:border-red-800 dark:bg-red-900"
                                        >
                    ...
                    <div className="bg-card rounded-lg border shadow-sm">
                        ...
                        {servedToday.length === 0 ? (
                            <p className="text-muted-foreground p-8 text-center text-sm">No orders served today yet</p>
                        ) : (
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b text-left">
                                            ...
                                            <th className="text-muted-foreground px-4 py-3 font-medium">Customer</th>
                                            <th className="text-muted-foreground px-4 py-3 font-medium">Items</th>
                                            <th className="text-muted-foreground px-4 py-3 font-medium">Total</th>
                                            <th className="text-muted-foreground px-4 py-3 font-medium">Payment</th>
                                            <th className="text-muted-foreground px-4 py-3 font-medium">Served At</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {servedToday.map((order) => (
                                            <tr key={order.id} className="border-b last:border-0">
                                                ...
                                                <td className="px-4 py-3">
                                                    <span
                                                        className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${paymentBadge(order)}`}
                                                    >
                                                        {paymentLabel(order)}
                                                    </span>
                                                </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowPayment(null)}>
                    <div className="bg-card w-full max-w-sm rounded-lg p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="text-muted-foreground mb-4 text-sm">
                            {showPayment.order_number} — ₱{formatPrice(showPayment.total)} via {showPayment.payment_method}
                        </p>
                        <form onSubmit={handleConfirmPayment} className="space-y-3">
                            {showPayment.payment_method === 'gcash' && (
                                <div>
                                    <label className="mb-1 block text-sm font-medium">GCash Reference #</label>
                                    <input
                                        value={gcashRef}
                            ...
                            {showPayment.payment_method === 'cash' && (
                                <div>
                                    <label className="mb-1 block text-sm font-medium">Cash Received (₱)</label>
                                    <input
                                        type="number"
                                        step="0.01"
                                        min={showPayment.total}
                                        value={cashReceived}
                                    ...
                                    {cashReceived && parseFloat(cashReceived) >= showPayment.total && (
                                        <p className="mt-1 text-sm text-green-600">
                                            Change: ₱{formatPrice(parseFloat(cashReceived) - showPayment.total)}
                                        </p>
                                    )}
                                </div>
                            )}
