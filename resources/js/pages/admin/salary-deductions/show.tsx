...
interface OrderRef { id: number; order_number: string; total: number; status: string; created_at: string }
interface Deduction {
    id: number; amount: number; payroll_month: string; payroll_year: number;
    created_at: string; order: OrderRef | null;
}

interface MonthlyBreakdown { label: string; total: number }
...
export default function SalaryDeductionShow() {
    ...
    const handleUpdateLimit = () => {
        router.patch(`/admin/salary-deductions/${employee.id}/limit`, {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-6 p-6">
                ...
                <div className="flex items-center gap-4">
                    <Link href="/admin/salary-deductions" className="rounded-md p-2 hover:bg-accent">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">{employee.employee_id} — {employee.department} — {employee.position}</p>
                    </div>
                </div>
                        ...
                        {editing ? (
                            <div className="mt-2 flex items-center gap-2">
                                ...
                                <input type="number" value={newLimit} onChange={e => setNewLimit(e.target.value)}
                                    className="w-32 rounded border bg-background px-2 py-1 text-lg font-bold" autoFocus />
                                <button onClick={handleUpdateLimit} className="rounded bg-primary px-3 py-1 text-sm text-primary-foreground">Save</button>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => { setEditing(true); setNewLimit(String(employee.limit)); }}
                                    className="rounded p-1 text-muted-foreground hover:bg-accent"><Edit2 className="h-4 w-4" /></button>
                            </div>
                        )}
                    </div>
                    <div className="rounded-lg border bg-card p-5 shadow-sm">
                        <p className="text-xs font-medium text-muted-foreground">Used ({currentMonth})</p>
                        <p className="text-2xl font-bold text-amber-600">₱{formatPrice(employee.used)}</p>
                        <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                            <div className={`h-full rounded-full transition-all ${usagePercent >= 100 ? 'bg-red-500' : usagePercent >= 80 ? 'bg-amber-500' : 'bg-green-500'}`}
                                style={{ width: `${Math.min(usagePercent, 100)}%` }} />
                        </div>
                        <p className="mt-1 text-xs text-muted-foreground">{usagePercent}% of limit</p>
                    </div>
                    <div className="rounded-lg border bg-card p-5 shadow-sm">
                        <p className="text-xs font-medium text-muted-foreground">Remaining</p>
                        <p className={`text-2xl font-bold ${employee.remaining <= 0 ? 'text-red-600' : 'text-green-600'}`}>₱{formatPrice(employee.remaining)}</p>
                        <p className="mt-1 text-xs text-muted-foreground">{deductions.data.length > 0 ? `${deductions.data.length} transactions this page` : 'No transactions'}</p>
                    </div>
                </div>
                            ...
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    <div className="h-6 w-full overflow-hidden rounded bg-gray-100 dark:bg-gray-800">
                                        <div className={`h-full rounded transition-all ${i === 0 ? 'bg-primary' : 'bg-primary/50'}`}
                ...
                <div className="rounded-lg border bg-card shadow-sm">
                    ...
                    {deductions.data.length === 0 ? (
                        <div className="p-8 text-center">
                            ...
                            <p className="mt-2 text-sm text-muted-foreground">No deduction transactions</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b text-left">
                                        ...
                                        <th className="px-4 py-3 font-medium text-muted-foreground">Payroll Period</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {deductions.data.map(d => (
                                        <tr key={d.id} className="border-b last:border-0">
                                            <td className="px-4 py-3">
                                                ...
                                                <p className="text-xs text-muted-foreground">{formatTime(d.created_at)}</p>
                                            </td>
                                            <td className="px-4 py-3">
                                                {d.order ? (
                                                    <Link href={`/orders/${d.order.id}`} className="font-medium text-primary hover:underline">
                    ...
                    {deductions.last_page > 1 && (
                        <div className="flex items-center justify-center gap-2 border-t px-4 py-3">
                            {Array.from({ length: deductions.last_page }, (_, i) => i + 1).map(page => (
                                <button key={page} onClick={() => router.get(`/admin/salary-deductions/${employee.id}`, { page }, { preserveState: true })}
