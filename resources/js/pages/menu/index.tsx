...
export default function MenuIndex() {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Menu" />
            <div className="flex flex-1 flex-col gap-4 p-3 pb-24 sm:p-4 sm:pb-6 lg:p-6">
                <div className="flex items-center justify-between gap-2">
                    <div className="flex-1 text-center">
                        <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Our Menu</h1>
                        <p className="text-muted-foreground mt-1 text-sm sm:text-base">Browse our selection of meals</p>
                    </div>
                    {itemCount > 0 && (
                        <button
                            onClick={() => setShowCart(!showCart)}
                            className="bg-primary text-primary-foreground hover:bg-primary/90 relative shrink-0 rounded-full p-2.5 shadow-lg transition-transform hover:scale-105 sm:p-3"
                        >
                            <ShoppingCart className="h-5 w-5 sm:h-6 sm:w-6" />
                            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white sm:h-6 sm:w-6 sm:text-xs">
                                {itemCount}
                            </span>
                        </button>
                    )}
                ...
                {showCart && itemCount > 0 && (
                    <div className="bg-card fixed inset-x-2 top-16 z-50 mx-auto max-w-sm rounded-lg border p-4 shadow-xl sm:inset-x-auto sm:top-20 sm:right-4 sm:left-auto sm:w-80">
                        <div className="mb-3 flex items-center justify-between">
                            <h3 className="font-semibold">Your Cart ({itemCount})</h3>
                            <button onClick={() => setShowCart(false)} className="hover:bg-accent rounded p-1">
                                <X className="h-4 w-4" />
                            </button>
                        </div>
                                ...
                                <div key={item.menuItem.id} className="flex items-center justify-between rounded border p-2 text-sm">
                                    ...
                                    <div className="ml-2 flex items-center gap-1">
                                        <button
                                            ...
                                            className="hover:bg-accent rounded border p-0.5"
                                        >
                        ...
                        <div className="mt-3 border-t pt-3">
                            ...
                            <a
                                href="/orders/create"
                                className="bg-primary text-primary-foreground hover:bg-primary/90 mt-2 block w-full rounded-md py-2 text-center text-sm font-medium"
                            >
                                Proceed to Checkout
                            </a>
                        </div>
                        ...
                        {categories.map((category) => (
                            <section key={category.id} className="flex flex-col gap-4">
                                    ...
                                    {category.menu_items.map((item) => {
                                        ...
                                        return (
                                            <Card key={item.id} className={`transition-all ${isSoldOut ? 'opacity-60' : 'hover:shadow-md'}`}>
                                                {item.image_url && (
                                                    <div
                                                        className={`aspect-video w-full overflow-hidden rounded-t-lg ${isSoldOut ? '' : 'cursor-pointer'}`}
                                                ...
                                                <CardHeader
                                                    className={isSoldOut ? '' : 'cursor-pointer'}
                                                    onClick={() => !isSoldOut && handleItemClick(item)}
                                                >
                                ...
                                {category.menu_items.length === 0 && (
                                    <p className="text-muted-foreground py-4">No items available in this category</p>
                                )}
                            </section>
                        ))}
                    </div>
                ...
                {itemCount > 0 && (
                    <div className="bg-card fixed right-0 bottom-0 left-0 z-40 border-t p-4 shadow-lg md:left-[var(--sidebar-width)]">
                        <div className="mx-auto flex max-w-4xl items-center justify-between">
                            <div>
                                <span className="font-semibold">
                                    {itemCount} item{itemCount > 1 ? 's' : ''}
                                </span>
                                <span className="text-muted-foreground ml-2">₱{formatPrice(total)}</span>
                            </div>
                            <a
                                href="/orders/create"
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="max-w-2xl">
                    ...
                    {selectedItem?.description && <p className="text-muted-foreground mb-4">{selectedItem.description}</p>}
